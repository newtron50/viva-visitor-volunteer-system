...
echo "
<br><br>
<div style='width:80%;'><center>Select a Day to view:
...
if(isset($todo) and $todo=="submit") {
  ...
  echo '<br><center>'.$vclass_2.' report for: '.$today;
  if (empty($_GET['print'])) {
    echo '<a href="search_today.php?print=y&t='.$today.'&vc='.$vclass.'" target="_blank" style="margin-left:50px;">Print Report</a></center>';
}
  $sql = "SELECT * FROM record_log where ORDER BY code_in ASC";
  if(!$result = $conn->query($sql)){
      die('There was an error running the query [' . $conn->error . ']');
  }
  echo '<table id="table" class="tablesorter" style="font-size:14px;"><thead><tr>';
