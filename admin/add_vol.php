...
echo '<form action="add_vol.php" method="post"><input type=hidden name=todo value="submit">
...
while($row=$result->fetch_assoc()){
      ...
      echo '<tr><td>'.$row['first_name'].'</td><td>'.$row['last_name'].'</td><td>'.$row['family_grp'].'</td><td><input name="grp4" type="checkbox" value="'.$temp_grp.'"/><label>Assign Group # to new user?</label></td></tr>';
      $x++;
    }  else {
      $temp_grp=$row['family_grp'];
      echo '<tr><td>'.$row['first_name'].'</td><td>'.$row['last_name'].'</td><td>'.$temp_grp.'</td><td><input name="grp5" type="checkbox" value="'.$temp_grp.'"/><label>Assign Group # to new user?</label></td></tr>';
    }
...
$chg=0;
echo '<br><br>Verify your entry<br><table><tr>';
...
echo'</td><td>';
  ...
  echo '<br><input type="submit" class="g_bubble" value="Create User"><span style="position:relative;margin-left:40px;"></span><a href="settings.php" class="g_bubble">CANCEL</a></center></form>';
